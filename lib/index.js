...
if (typeof WebSocket === 'undefined') {
  var WebSocket = require('sockjs-client')
}
...
function robot (address, opts) {
  ...
  ws.onerror = function (msg) {
    ws.close()
    throw new Error(`Could not connect to the robot at ${address}`)
  }
  ...
  function send (data) {
    if (lastWrite && lastWrite.command === 'run-forever' && deepEqual(data, lastWrite)) {
      return
    }
    lastWrite = data
    ws.send(JSON.stringify({
